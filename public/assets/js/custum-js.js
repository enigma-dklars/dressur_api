$(document).ready(function () {
    ...
    // ── BOOST CONTACT : helpers ──────────────────────────────────────────
    var getTypeBoostActif = function () {
        return $('#typeBoostQuota').hasClass('btn-primary') ? 'quota' : 'date';
    };

    var updateBoostDescription = function () {
        if (!$('#boostDescription').length) { return; }
        var type   = getTypeBoostActif();
        // .prop('hidden') retourne un vrai booléen (true/false) quel que soit
        // le mode de définition de l'attribut HTML (hidden / hidden="true" / removeAttr)
    ...
    if ($('#boostDescription').length) {
        // Affichage immédiat avec valeurs par défaut (pas d'attente réseau)
        updateBoostDescription();
        // Mise à jour avec les vraies valeurs de la DB
        $.get('/api/freeBoostInfo')
            .done(function (data) {
    ...
    $(document).on('click', '#typeBoostDate', function () {
        ...
        updateBoostDescription();
        if (!$('#boostContactPayant').prop('hidden')) { chargerFormuleBoost(); }
    });
    ...
    $(document).on('click', '#modeBoostPayant', function () {
        ...
        $('#boostContactGratuit').prop('hidden', true);
